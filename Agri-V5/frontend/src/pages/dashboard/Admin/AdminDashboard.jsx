  ...
  useEffect(() => {
    ...
    socket.on("sensorData", data => {
      ...
      setDeviceStatus(ds =>
        ds.map(d =>
          data.device === d.name
            ? { ...d, status: data.status, time: new Date().toLocaleTimeString() }
            : d
        )
      );
    });
  ...
  return (
    <>
      ...
      <section className="relative h-[80vh] flex flex-col justify-center items-center text-white px-8 text-center overflow-hidden bg-gradient-to-tr from-stone-800 via-lime-700 to-amber-400">
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2 }} className="max-w-4xl bg-white bg-opacity-20 backdrop-blur-md rounded-3xl p-12 shadow-xl">
          <h1 className="text-5xl font-extrabold mb-6 drop-shadow-lg">🌾 SmartFarm Admin Dashboard</h1>
          <p className="text-xl drop-shadow-md">Monitor your agricultural fields with real-time sensor data, IoT control, and analytics — all in one place.</p>
        </motion.div>
      </section>
      ...
      <main className="bg-gray-50 p-12 space-y-20 min-h-screen">
        ...
        <div className="flex justify-between items-center max-w-6xl mx-auto">
          ...
          <motion.button whileTap={{ scale: 0.95 }} whileHover={{ scale: 1.05 }} onClick={handleRefresh} className="flex items-center gap-3 bg-emerald-700 text-white px-7 py-3 rounded-full shadow-lg hover:bg-emerald-800">
            <RefreshCcw size={20} /> Refresh Data
          </motion.button>
        </div>

        {/* Sensor Cards */}
        <motion.div initial="hidden" animate="visible" variants={{ hidden:{opacity:0,y:30}, visible:{opacity:1,y:0,transition:{staggerChildren:0.2,delayChildren:0.15}} }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 max-w-6xl mx-auto">
          ...
          <AnimatedCard><SensorCard label="Total Users" value={userCount} icon={Users} color="purple" /></AnimatedCard>
        </motion.div>
        ...
        <Card className="bg-white p-8 shadow-2xl rounded-3xl border border-gray-300 max-w-6xl mx-auto">
          <CardContent>
            ...
            <p className="text-gray-700 mb-8">Real-time insights from your fields.</p>
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={chartData}>
                ...
                <Tooltip contentStyle={{ backgroundColor:"#f0fdf4", borderRadius:"10px", borderColor:"#a7f3d0", color:"#065f46", fontWeight:"600" }} />
        ...
        <section className="bg-gray-50 py-12 max-w-6xl mx-auto rounded-3xl shadow-xl border border-gray-200">
          <h2 className="text-3xl font-bold text-emerald-700 mb-6">📦 IoT Devices Status</h2>
              ...
              <tbody>
                {deviceStatus.map((d,i) => (
                  <tr key={i} className="border-b hover:bg-gray-100 transition">
                    ...
                    <td className={`px-6 py-4 font-bold ${d.status === "Online" ? "text-green-600" : d.status === "Offline" ? "text-red-500" : "text-yellow-500"}`}>{d.status}</td>
        ...
        <section className="bg-white py-12 max-w-6xl mx-auto rounded-3xl shadow-xl border border-amber-200">
          <h2 className="text-3xl font-bold text-amber-700 mb-6">📋 Recent System Events</h2>
          <ul className="space-y-4">
            {events.map((e,i) => (
              <li key={i} className="bg-gray-50 border-l-4 border-amber-500 shadow p-4 rounded">{e}</li>
            ))}
            {events.length === 0 && <li className="text-gray-500">No events recorded.</li>}
          </ul>
        </section>
        ...
        <section className="bg-gray-50 py-12 max-w-6xl mx-auto rounded-3xl shadow-xl border border-gray-200">
          ...
          <div className="flex items-center gap-6">
            <motion.button whileTap={{ scale: 0.95 }} whileHover={{ scale: 1.05 }} onClick={triggerPump} className="flex items-center gap-2 bg-amber-600 text-white px-6 py-3 rounded-full shadow hover:bg-amber-700">
        ...
        {/* Yield / Harvest History */}
        <section className="bg-white py-12 max-w-6xl mx-auto rounded-3xl shadow-xl border border-gray-200">
          <h2 className="text-3xl font-bold text-emerald-800 mb-4">📈 Yield & Harvest History</h2>
          <p className="text-gray-600 mb-8">Track your productivity over time.</p>
